...
import { CustomPrimitive, generateData } from './data/cloud'
...
function initMap() {
  ...
  const viewer = new Cesium.Viewer('cloud-map', {
    infoBox: false,
    shouldAnimate: true,
    vrButton: false,
    geocoder: false,
...
function initGUI() {
  ...
  gui.add(customPrimitive.viewModel, 'opacity', 0, 1, 0.01).onChange(updateValue)
  gui.add(customPrimitive.viewModel, 'range', 0, 1, 0.01).onChange(updateValue)
  gui.add(customPrimitive.viewModel, 'frame', 0, 100, 1).onChange(updateValue)
}
